...
| ID | Parte | Entrega principal | Dependência | Situação | Responsável | Branch |
|---|---|---|---|---|---|---|
| P18 | Banco de dados inicial | Estrutura segura para imóveis, pessoas, vínculos, veículos e usuários | Regras e arquitetura aprovadas | 🟡 Em andamento — fundação multi-implantação, grupo Imóveis e grupo Pessoas (`pessoas`, `pessoa_documentos`, `pessoa_contatos`, `pessoa_enderecos`) entregues, todos com testes de isolamento e concorrência otimista; `pessoa_documentos.valor_normalizado` sem cifra (decisão provisória, pendente de `PEN-BDD-007`); `pessoa_arquivos` fora de escopo até o grupo de arquivos/S3 existir. Faltam Vínculos, Veículos e Usuários/Perfis/Permissões antes de marcar ✅ Concluída | Vinicius | [`vinicius/p18-fundacao-imoveis`](https://github.com/remotevelaskoo/SDV-SANTARITA/pull/22), [`vinicius/p18-pessoas`](https://github.com/remotevelaskoo/SDV-SANTARITA/pull/23) |
