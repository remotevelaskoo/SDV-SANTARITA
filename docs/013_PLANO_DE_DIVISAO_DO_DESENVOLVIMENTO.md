...
|---|---|---|---|---|
| AJ-001 — Conferência visual protegida | Porteiro/Caixa autorizado abre a foto, selfie e documento efetivamente submetidos no detalhe do pré-cadastro e da validação; acesso privado e auditado | Antes da homologação interna dos fluxos de pré-cadastro e validação | Armazenamento privado, P20 e P22 | 🟡 Em andamento — fatia 1 concluída |
...
**AJ-001, fatia 1 concluída** — motivada por uma falha real observada em preparação de demonstração para investidor: o sistema não exibia foto da pessoa em nenhuma tela (moto portaria, validação de entrada, pré-cadastro). Entregue: entidade `Arquivo` genérica (`fileable` polimórfico, reutilizável para outras categorias no futuro), upload real de foto ligado ao cadastro direto na portaria (`PersonRegistration.php`, campo já existente na Etapa 2, antes decorativo), exibição da foto em Validação de entrada e Modo Portaria (com fallback para iniciais quando não há foto), nova permissão `validacao.visualizar-imagem` (Porteiro/Caixa, Gestor e Administrador — não Auditor, mesmo padrão de `validacao.registrar`) e trilha de auditoria escopada (`arquivo_acessos`, RN-046 a RN-049) a cada abertura de arquivo protegido. **Corte pragmático em relação ao ADR-006** (Aprovado, mas assume um fornecedor S3-compatível ainda não escolhido, sem credenciais em `.env`): o armazenamento usa o disco `local` do Laravel 13 — já privado por padrão (`storage/app/private`, não symlinkado para `public/`) — servido por rota autenticada e permissionada (`/arquivos/{arquivo}`), não uma URL pública; a chave segue o template opaco do ADR-006 §12 (`implantação/categoria/ano/mês/uuid`, sem id da pessoa, nome ou documento). Trocar para S3 é mudança de config no disco, não reescrita, já que a app usa a mesma abstração `Storage` que o ADR-006 pressupõe. Sem quarentena, scan de malware ou versionamento (ADR-006 completo) nesta fatia — validação síncrona no upload (mimetype real, extensão, tamanho máx. 5 MB), arquivo já nasce `disponível`. **Fica para fatias futuras**: foto/documento no pré-cadastro público (`PublicPreRegistration.php`) e no detalhe da fila de análise (`PreRegistrationQueue.php`) — hoje esse fluxo nem promove um `PreRegistration` aprovado para uma `Pessoa` real, então a ponte também está pendente; AJ-002 (revelação controlada de CPF) é mecanismo independente (mascaramento + revelação temporária + permissão própria), sem relação com armazenamento de arquivo.
