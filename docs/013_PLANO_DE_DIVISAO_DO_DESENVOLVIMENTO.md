...
| ID | Parte | Entrega principal | Dependência | Situação | Responsável | Branch |
|---|---|---|---|---|---|---|
| P18 | Banco de dados inicial | Estrutura segura para imóveis, pessoas, vínculos, veículos e usuários | Regras e arquitetura aprovadas | 🟡 Em andamento — fundação multi-implantação, grupo Imóveis, grupo Pessoas e grupo Vínculos (`vinculos`, `imovel_responsabilidades`) entregues, todos com testes de isolamento e concorrência otimista; `imovel_responsabilidades` fecha a lacuna deixada no PR #22; `vinculos.tipo`/`papel` sem catálogo formal (decisão provisória, pendente de `PEN-BDD-003`); vínculo com empresa fora de escopo até o grupo Empresas existir como tabela real; dados de demonstração ligam pessoas e imóveis já semeados (responsável principal). Faltam Veículos e Usuários/Perfis/Permissões antes de marcar ✅ Concluída | Vinicius | [`vinicius/p18-fundacao-imoveis`](https://github.com/remotevelaskoo/SDV-SANTARITA/pull/22), [`vinicius/p18-pessoas`](https://github.com/remotevelaskoo/SDV-SANTARITA/pull/23), [`vinicius/p18-vinculos`](https://github.com/remotevelaskoo/SDV-SANTARITA/pull/26) |
